...
import academicStructure from '../constants/academicStructure'; // قد تحتاج إلى بعض الثوابت هنا أيضاً
import { ToastContext } from '../App';
import LoginIllustration from '../assets/images/login_illustration.png'; // صورة توضيحية لصفحة تسجيل الدخول
...
function TeamRegistrationPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        user_type: 'team_member', // ثابت لصفحة عضو فريق العمل
    });
    ...
    const handleInputChange = (e) => {
        ...
        if (errors[name]) { // مسح خطأ الحقل المحدد عند تغييره
    ...
    const handleSubmit = async (e) => {
        ...
        // NEW: تغيير مسار API التسجيل ليتوافق مع مسار Django الصحيح
        const apiEndpoint = 'http://127.0.0.1:8000/api/register/team/';

        try {
            const response = await axios.post(apiEndpoint, formData); // FormData غير ضرورية هنا إن لم تكن ترفع ملفات
                        ...
                        const fieldErrors = Object.entries(err.response.data)
                            .map(([field, messages]) => {
                                const fieldName = {
                                    ...
                                    'previous_work_experience': 'خبرة العمل السابقة', 'non_field_errors': ''
                                }[field] || field;
                    ...
                    <div className="container registration-container">
                        ...
                        <div className="registration-form-wrapper">
                            ...
                            <div className="team-form-sections-nav">
                                ...
                                {/* أزل هذا الزر إذا كان هناك قسمين فقط */}
                            ...
                            <form className="team-register-form" onSubmit={handleSubmit}>
                                ...
                                {currentSection === 2 && (
                                    <div className="form-section-content active-section">
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input type="number" id="expected_salary" name="expected_salary" value={formData.expected_salary} onChange={handleInputChange} step="0.01" />
                                        </div>
                                ...
                                {/* القسم الأخير: رفع المستندات (معطل حالياً، يمكن تفعيله عند الحاجة) */}
                                {/* {currentSection === 3 && (
                                    <div className="form-section-content active-section">
