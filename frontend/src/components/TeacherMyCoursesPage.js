...
import Course1 from '../assets/images/course1.jpg'; // Placeholder for course image
...
function TeacherMyCoursesPage() {
    ...
    useEffect(() => {
        ...
        const fetchCourseAndLectures = async () => {
            ...
            try {
                ...
                // مسار API للمحاضرات (داخل كورس معين)
    ...
    const handleUpdateMaterial = async (materialId, updatedData, lectureIndex) => {
        setLoading(true);
        try {
            await axios.put(`http://127.00.0.1:8000/api/materials/${materialId}/`, updatedData, {
                headers: { 'Authorization': `Token ${userToken}` }
            });
