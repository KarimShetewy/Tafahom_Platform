...
const academicStructure = {
    'first_secondary': {
        ...
        tracks: {
            'general': { label: 'عام (الصف الأول الثانوي)' }
        },
        all_materials: [
    ...
    'second_secondary': {
        ...
        tracks: {
            'scientific': { label: 'علمي (الصف الثاني الثانوي)' },
            'literary': { label: 'أدبي (الصف الثاني الثانوي)' },
        },
        all_materials: [
    ...
    'third_secondary': {
        ...
        tracks: {
            'science_sec_3': { label: 'علمي علوم (الصف الثالث الثانوي)' },
            'math_sec_3': { label: 'علمي رياضة (الصف الثالث الثانوي)' },
            'literary_sec_3': { label: 'أدبي (الصف الثالث الثانوي)' },
        },
        all_materials: [
            'arabic_lang', 'english_lang', 'french_lang', 'german_lang',
...
for (const levelKey in academicStructure) {
    if (academicStructure.hasOwnProperty(levelKey) && levelKey !== 'allSubjectsMap') {
        // التأكد أن all_materials موجودة قبل محاولة عمل map عليها
        if (academicStructure[levelKey].all_materials) { 
